...
func findName(parts []string, candidates map[string]bool) string {
	for _, part := range parts { if candidates[part] { return part } }
	return ""
}

func findPathRole(directories []string, stem string) string {
	if role := findName(directories, roleNames); role != "" { return role }
	for role := range roleNames { if stem == role || strings.HasSuffix(stem, "_"+role) { return role } }
	return ""
}

func findRole(parts []string) string {
	for _, part := range parts {
		if roleNames[part] { return part }
		if strings.HasSuffix(part, "commander") { return "commander" }
		if strings.HasSuffix(part, "messenger") { return "messenger" }
		if strings.HasSuffix(part, "processing") { return "processing" }
		if strings.HasSuffix(part, "compresser") { return "compresser" }
	}
	return ""
}

func findApplication(parts []string) string {
	for index, part := range parts { if appRoots[part] && index+1 < len(parts) { return parts[index+1] } }
	return ""
}
