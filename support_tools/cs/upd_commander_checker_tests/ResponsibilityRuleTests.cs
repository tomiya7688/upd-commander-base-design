...
public sealed class ResponsibilityRuleTests
{
    ...
    public void ResponsibilityRulesHaveExpectedCodes()
    {
        ...
        project.Write(
            "process/large_processing.cs",
            $"namespace Sample; internal sealed class LargeProcessing {{ {methods} }}"
        );
        project.Write(
            "process/mixed_processing.cs",
            "namespace Sample; internal sealed class First { internal void Run() { } } internal sealed class Second { internal void Run() { } }"
        );
