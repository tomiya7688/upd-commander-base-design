...
bool excluded_common_usage_path(const std::string& path) {
    for (const auto& part : std::filesystem::path(path)) {
        const std::string name = lower_name(part);
        if (name == "test" || name == "tests" || name == "__tests__" ||
            name == "generated" || name == "autogenerated") {
            return true;
        }
    }
    return false;
...
bool generated_source(const std::filesystem::path& path) {
    std::ifstream source(path);
    std::string line;
    for (int index = 0; index < 5 && std::getline(source, line); ++index) {
        if (line.find("generated") != std::string::npos || line.find("Generated") != std::string::npos) {
            return true;
        }
    }
    return false;
...
std::vector<Finding> scan_path(
    ...
    std::unordered_map<std::string, std::vector<std::string>> common_assets;
    std::unordered_map<std::string, std::unordered_map<std::string, bool>> common_references;
    for (const auto& path : paths) {
        const std::string relative = relative_text(path, root);
        if (!ignored_by_cli(relative, cli_ignore) && !is_path_ignored(relative, rules) &&
            !excluded_common_usage_path(relative) && !generated_source(path) &&
            classify_path(relative, common_roots).layer == "common") {
            common_assets[relative] = {};
        }
    }
    for (const auto& path : paths) {
        const std::string relative = relative_text(path, root);
        ...
        std::vector<std::string> included_targets;
        auto current = analyze_cpp_ast(
            path, root, relative, rules, upd301_max_inputs, common_roots, &included_targets);
        findings.insert(findings.end(), current.begin(), current.end());
        const auto source_layer = classify_path(relative, common_roots).layer;
        if (!excluded_common_usage_path(relative) && !generated_source(path) &&
            (source_layer == "ui" || source_layer == "process" || source_layer == "data")) {
            for (const auto& included : included_targets) {
                if (common_assets.find(included) != common_assets.end()) {
                    common_references[included][source_layer] = true;
                }
            }
        }
    ...
    for (const auto& asset : common_assets) {
        const auto refs = common_references.find(asset.first);
        if (refs == common_references.end() || refs->second.size() != 1 ||
            is_ignored(asset.first, "UPD407", first_line(root / asset.first), rules)) {
            continue;
        }
        findings.push_back(Finding{
            asset.first,
