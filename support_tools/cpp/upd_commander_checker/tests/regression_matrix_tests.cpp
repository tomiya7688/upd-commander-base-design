...
void test_common_usage_warning_matrix() {
    const auto one_layer = std::filesystem::temp_directory_path() / "upd_cpp_common_usage_one";
    std::filesystem::remove_all(one_layer);
    write_file(one_layer / "common" / "contracts" / "message.hpp", "struct Message {};\n");
    write_file(
        one_layer / "ui" / "view.cpp",
        "#include \"../common/contracts/message.hpp\"\nMessage make_message() { return {}; }\n");
    const auto one_findings = upd_checker::scan_path(one_layer.string(), {});
    assert(has_code_message(one_findings, "UPD407", "Common/Shared asset"));
    std::filesystem::remove_all(one_layer);

    const auto multiple_layers = std::filesystem::temp_directory_path() / "upd_cpp_common_usage_multi";
    std::filesystem::remove_all(multiple_layers);
    write_file(multiple_layers / "common" / "contracts" / "message.hpp", "struct Message {};\n");
    write_file(
        multiple_layers / "ui" / "view.cpp",
        "#include \"../common/contracts/message.hpp\"\nMessage make_message() { return {}; }\n");
    write_file(
        multiple_layers / "process" / "worker.cpp",
        "#include \"../common/contracts/message.hpp\"\nMessage make_message() { return {}; }\n");
    assert(!has_code(upd_checker::scan_path(multiple_layers.string(), {}), "UPD407"));
    std::filesystem::remove_all(multiple_layers);

    const auto unused = std::filesystem::temp_directory_path() / "upd_cpp_common_usage_unused";
    std::filesystem::remove_all(unused);
    write_file(unused / "common" / "contracts" / "message.hpp", "struct Message {};\n");
    assert(!has_code(upd_checker::scan_path(unused.string(), {}), "UPD407"));
    std::filesystem::remove_all(unused);
}

void test_common_usage_exclusions_ignore_and_custom_root() {
    const auto excluded = std::filesystem::temp_directory_path() / "upd_cpp_common_usage_excluded";
    std::filesystem::remove_all(excluded);
    write_file(excluded / "common" / "contracts" / "message.hpp", "struct Message {};\n");
    write_file(
        excluded / "ui" / "tests" / "view.cpp",
        "#include \"../../common/contracts/message.hpp\"\nMessage make_message() { return {}; }\n");
    assert(!has_code(upd_checker::scan_path(excluded.string(), {}), "UPD407"));
    write_file(
        excluded / "ui" / "generated_view.cpp",
        "// Generated by fixture\n#include \"../common/contracts/message.hpp\"\nMessage make_message() { return {}; }\n");
    assert(!has_code(upd_checker::scan_path(excluded.string(), {}), "UPD407"));
    std::filesystem::remove_all(excluded);

    const auto custom = std::filesystem::temp_directory_path() / "upd_cpp_common_usage_custom";
    std::filesystem::remove_all(custom);
    write_file(custom / "contracts" / "message.hpp", "struct Message {};\n");
    write_file(
        custom / "ui" / "view.cpp",
        "#include \"../contracts/message.hpp\"\nMessage make_message() { return {}; }\n");
    auto findings = upd_checker::scan_path(custom.string(), {}, 2, 12, 80, 3, 2, {"contracts"});
    assert(has_code(findings, "UPD407"));
    write_file(custom / ".updcommanderignore", "UPD407 contracts/**\n");
    findings = upd_checker::scan_path(custom.string(), {}, 2, 12, 80, 3, 2, {"contracts"});
    assert(!has_code(findings, "UPD407"));
    std::filesystem::remove_all(custom);
}
...
int main() {
    ...
    test_common_usage_warning_matrix();
    test_common_usage_exclusions_ignore_and_custom_root();
    return 0;
}
