...
クラスを使用する言語では、1つの主要クラスは原則として1つの責務だけを持つものとします。

クラスが複数の独立した理由で変更される状態になった場合、そのクラスは責務過多です。

UPD Commander では、処理量が増えても Commander が別の Processing や Commander を順番に呼び出して処理を組み合わせられます。そのため、処理量の増加を理由に1クラスへ複数責務を詰め込む必要はありません。

また、UPDを厳密に守っていれば、処理量が大きくなったクラスやモジュールは責務過多の有力な兆候として扱えます。

静的チェッカーでは意味上の責務を完全には断定できないため、次の機械的兆候を warning として検出します。

- `UPD401`: クラス、型、ファイル等の責務単位が大きすぎる。
- `UPD402`: 1ファイルに複数の主要クラスが定義されている。

`record`、DTO、enum、単純な値型など、処理責務を持たない補助型は `UPD402` の対象外にできます。
